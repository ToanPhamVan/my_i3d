...
def images_loader(images_path, transform=None):
    ...
    images_list = [
        ...
        if not i.stem.startswith(".") and i.suffix.lower() in _IMAGE_EXT
    ]
...
class FrameGenerator(object):
    def __init__(self, input_path, resize=None):
        """
    ...
    def get_frame(self):
        frame = self.frames[
            self.counter
        ]  # cv2.resize(frame, (_IMAGE_SIZE, _IMAGE_SIZE))
        self.counter += 1
        return frame
...
def mass_process(opts):
    ...
    with mp.Pool() as pool:
        for result in tqdm(
            pool.imap_unordered(pre_process, param_list), total=len(param_list)
        ):
            pass
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Pre-process the video into formats which i3d uses."
    )
    parser.add_argument(
        "--data_root",
        type=str,
